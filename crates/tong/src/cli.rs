...
fn gc(args: &[String]) -> Result<()> {
    ...
    let filtered: Vec<String> = args.iter().filter(|a| *a != "--dry-run").cloned().collect();
    let options = BuildCliOptions::parse(&filtered)?;
    let manifest_path = Manifest::discover(&options.path)?;
    let manifest = Manifest::load(&manifest_path)?;
    let out_dir = manifest.root.join("target/tong");
    let (scanned, removed) = build_state::gc(&out_dir, dry_run)?;
    if dry_run {
        println!(
            "would remove {removed} of {scanned} files under {}",
            out_dir.display()
        );
    } else {
        println!(
            "removed {removed} of {scanned} files under {}",
            out_dir.display()
        );
    }
    Ok(())
...
COMMANDS:
  ...
  gc        Delete untracked files under target/tong
