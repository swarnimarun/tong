...
fn rust_lib_output_name(crate_name: &str, hash: &str, proc_macro: bool) -> String {
    if !proc_macro {
        return format!("lib{crate_name}-{hash}.rlib");
    }

    if cfg!(windows) {
        format!("{crate_name}-{hash}.dll")
    } else if cfg!(target_os = "macos") {
        format!("lib{crate_name}-{hash}.dylib")
    } else {
        format!("lib{crate_name}-{hash}.so")
    }
}

fn add_feature_args(features: &BTreeSet<String>, args: &mut Vec<String>) {
...
fn parse_build_script_stdout(stdout: &str) -> BuildScriptOutput {
    ...
    for line in stdout.lines() {
        ...
        match key {
            "rustc-cfg" => output.cfgs.push(value.to_owned()),
            "rustc-env" => {
                if let Some((name, value)) = value.split_once('=') {
                    output.rustc_env.insert(name.to_owned(), value.to_owned());
                }
            }
            "rustc-link-search" => output.link_search.push(value.to_owned()),
            "rustc-link-lib" => output.link_libs.push(value.to_owned()),
            _ => {}
        }
    }
...
fn build_script_env(build_script: Option<&BuildScriptOutput>) -> BTreeMap<String, String> {
    let mut env = BTreeMap::new();
    if let Some(build_script) = build_script {
        env.insert(
            "OUT_DIR".to_owned(),
        ...
        env.extend(build_script.rustc_env.clone());
    }
    env
}

fn add_build_script_args(build_script: Option<&BuildScriptOutput>, args: &mut Vec<String>) {
...
fn add_profile_args(profile: BuildProfile, args: &mut Vec<String>) {
    match profile {
        BuildProfile::Debug => {
            args.push("-C".to_owned());
...
fn opt_level(profile: BuildProfile) -> &'static str {
    match profile {
...
fn add_dependency_args(dependencies: &[(String, BuiltLib)], args: &mut Vec<String>) {
