...
| [Fine-grained cache locking](https://github.com/rust-lang/cargo/issues/4282) | concurrent builds | `[partial/planned: §10.3]` | same-workspace builds are serialized for correctness today; add digest-aware in-flight action claims so compatible builds wait/reuse work and divergent subgraphs run independently | Phase 1/0.3 |
